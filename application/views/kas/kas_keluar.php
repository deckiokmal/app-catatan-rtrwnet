                            ...
                            <tbody>
                                ...
                                <?php foreach ($kas_keluar as $k) : ?>
                                    <tr>
                                        <th scope="row"><?= $i; ?></th>
                                        <td><?= $k['nama_kas']; ?></td>
                                        <td>Rp. <?= number_format($k['jumlah_kas_keluar'], 2, ',', '.'); ?></td>
                                        <td><?= date('d F Y', strtotime($k['date_input'])); ?></td>
                                        <td><?= $k['ket_kas']; ?></td>
                                        <td>
                                            <div class="kt-section__content kt-section__content--solid" id="edit">

                                                <a href="" class="btn btn-icon btn-primary btn-success ubahKasKel" data-toggle="modal" data-target="#KasKel" data-id="<?= $k['id']; ?>"><span class="tags" data-toggle="kt-tooltip" data-placement="left" title="Edit"><i class=" fa fa-pencil"></i></span></a>
                                                <a href="<?= base_url(''); ?>kas/hapusKasKel/<?= $k['id']; ?>" class="btn btn-icon btn-primary btn-danger tombol-hapus"><i class=" fa fa-trash"></i> </a>
...
<!--begin::Modal-->
<div class="modal fade" id="KasKel" tabindex="-1" role="dialog" aria-labelledby="kasKelModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="kasKelModalLabel">Tambah Kas Keluar</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                </button>
            </div>
            <div class="modal-body">
                <form action="<?= base_url('kas/kas_keluar'); ?>" method="post">
                    <input type="hidden" name="id" id="id">
                    <div class="form-group">
                        <label for="recipient-name" class="form-control-label">Nama Kas Keluar :</label>
                        <input type="text" class="form-control" id="nama_kas_keluar" name="nama_kas_keluar" placeholder="Masukan Nama Kas Keluar">
                    </div>
                    <div class="form-group">
                        <label for="recipient-name" class="form-control-label">Jumlah Kas Keluar :</label>
                        <input type="text" class="form-control" id="jumlah_kas_keluar" name="jumlah_kas_keluar" placeholder="Jumlah Kas Keluar">
                    </div>
                    <div class="form-group">
                        <label for="recipient-name" class="form-control-label">Keterangan Kas Keluar :</label>
                        <input type="text" class="form-control" id="ket_kas_keluar" name="ket_kas_keluar" placeholder="Keterangan Kas Keluar">
                    </div>
